...
module.exports = {
  ...
  // Saves a browser tab as a numbered .url shortcut in the drop-target folder,
  // no prompt — mirroring opt+cmd+click's saveNumberedLink. Files are prefixed
  // `001-`, `002-`, … (zero-padded so they alpha-sort), continuing from the
  // highest NNN- number already present in that folder. The base name is the
  // tab's page title when we can find it, else the URL hostname.
  saveUrlToTree(url, targetDir) {
    ...
    try {
      ...
      atom.notifications.addError(`Could not read folder: ${error.message}`);
      return;
    }
    ...
    const base = sanitizeName(item?.title || deriveUrlTitle(url)) || 'link';

    const dest = path.join(targetDir, `${prefix}-${base}.url`);
    try {
